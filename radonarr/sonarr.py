...
class Sonarr(Client):
    ...
    async def queue(self) -> List[EpisodeQueueItem]:
        """Get currently downloading info."""
        results = await self._request("queue")

        return [EpisodeQueueItem.from_dict(result) for result in results]
    ...
    async def wanted(
        ...
        return EpisodeWantedResults.from_dict(results)

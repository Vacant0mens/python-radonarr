...
class Client:
    ...
    async def _request(
        ...
        try:
            ...
            raise RadonarrConnectionError(
                f"Timeout occurred while connecting to API at {url}"
            ) from exception
        except (aiohttp.ClientError, SocketGIAError) as exception:
            raise RadonarrConnectionError(
                f"Error occurred while communicating with API at {url}"
            ) from exception

        if response.status == 403:
            raise RadonarrAccessRestricted(
                f"Access restricted. Please ensure your API Key is provided and valid for: {url}", {}
            )

        if response.status == 404:
            raise RadonarrResourceNotFound(f"Resource not found: {url}")
        ...
        if (response.status // 100) in [4, 5]:
            ...
            if content_type == "application/json":
                raise RadonarrError(
                    f"{self.host}:{self.port} Response:\nHTTP {response.status}", json.loads(content.decode("utf8"))
                )
